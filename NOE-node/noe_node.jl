...
export NOutputError, NOE, averageEnergy, slug

"""
Description:

    A Nonlinear Output Error model.

    s_t = f(s_t-1, …, s_t-M, u_t, u_t-1, …, u_t-N)
    y_t = s_t + e_t

    where M is the auto-regression order of states and N of inputs.
    Concatenate previous states: x_t-1 = [s_t-1, …, s_t-M]' and previous
    inputs z_t-1 = [u_t-1, …, u_t-N]'.
...
Construction:
    ...
    NOutputError(y, θ, x, z, u, τ, g=Function, id=:some_id)
"""

mutable struct NOutputError <: SoftFactor
    id::Symbol
    ...
    function NOutputError(y, θ, x, z, u, τ; g::Function=x->sum(x), id=generateId(NOutputError))
...
slug(::Type{NOutputError}) = "NOE"

function averageEnergy(::Type{NOutputError},
